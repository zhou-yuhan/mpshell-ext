...
DroppingPacketQueue::DroppingPacketQueue( const json & args )
...
bool DroppingPacketQueue::can_accept_one( unsigned int pkt_size ) const
{
    bool ret = true;

    if ( byte_limit_ ) {
        ret &= ( size_bytes() + pkt_size <= byte_limit_ );
    }

    if ( packet_limit_ ) {
        ret &= ( size_packets() + 1 <= packet_limit_ );
    }
...
unsigned int DroppingPacketQueue::get_arg(const nlohmann::json & args, const std::string & name)
{
    return args.contains(name) ? static_cast<unsigned int>(args[name]) : 0;
}