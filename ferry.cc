...
int packet_ferry(FerryType& ferry, FileDescriptor& tun,
                 FileDescriptor& sibling_fd, unique_ptr<DNSProxy>&& dns_proxy,
                 ChildProcess&& child_process) {
    vector<ChildProcess> children;
    children.emplace_back(move(child_process));

    return packet_ferry(ferry, tun, sibling_fd, move(dns_proxy),
                        move(children));
}
...
int packet_ferry(FerryType& ferry, FileDescriptor& tun,
    ...
    poller.add_action(Poller::Action(tun, Direction::In, [&]() {
        // std::string packet = tun.read_packet();
        // if (!packet.empty()) {
        //     ferry.read_packet(packet);
        // }
    ...
    poller.add_action(Poller::Action(signal_fd.fd(), Direction::In, [&]() {
        return handle_signal(signal_fd.read_signal(), child_processes);
    }));

    /* add dns proxy TCP and UDP sockets */
    if (dns_proxy) {
        poller.add_action(Poller::Action(dns_proxy->udp_listener().fd(),
                                         Direction::In, [&]() {
                                             dns_proxy->handle_udp();
                                             return ResultType::Continue;
                                         }));

        poller.add_action(Poller::Action(dns_proxy->tcp_listener().fd(),
                                         Direction::In, [&]() {
...
template <class FerryType>
int packet_ferry(FerryType& ferry, FileDescriptor& tun,
                 FileDescriptor& sibling_fd, FileDescriptor& server_fd,
    ...
    /* server gets datagram -> accept connection
     * (register client request callback) */
    poller.add_action(Poller::Action(server_fd, Direction::In, [&]() {
        ferry.new_connection(poller);
        return ResultType::Continue;
    }));
